...
public class EHCacheUtil {
    ...
    public static String ehcachePath = "/config/ehcache.xml";
    public static String initCache = "role_menu_code";
    static{  
        EHCacheUtil.initCacheManager(ehcachePath);  
        EHCacheUtil.initCache(initCache);  
    }  
    /** 
     *  
     * 初始化缓存管理容器 
	...
	public static void main(String[] arg) {  
        // 初始化--必须  
    	EHCacheUtil.initCache = "uiuiui";
        EHCacheUtil.initCacheManager(EHCacheUtil.ehcachePath);  
        EHCacheUtil.initCache(EHCacheUtil.initCache);  
        ...
        for(int i=0;i<keys.size();i++){  
            System.out.println(keys.get(i)+"---------------"+EHCacheUtil.get(keys.get(i)));  
        }  
        EHCacheUtil.remove("F");
        System.out.println(EHCacheUtil.get("F"));  
        List keys1 = EHCacheUtil.getKeys();  
        for(int i=0;i<keys1.size();i++){  
            System.out.println(keys1.get(i)+"---------------"+EHCacheUtil.get(keys1.get(i)));  
        }  
