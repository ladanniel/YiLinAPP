		                                ...
		                                <select data-placeholder="选择载货车辆类型" id="stock" class="chosen-select form-control" multiple>
		                                	<@truckType>
		                        				<#list truckTypeViews as vo>
		                        				    <#list stockTypeNameList as sn>
		                        				    	<#if vo.name == sn>
		                        				    		<option value="${vo.id}" hassubinfo="true" selected = "selected">${vo.name}</option>
		                        				    	<#else>
	        ...
	        window.mySourceReady=function(options,source){
        		...
        		if(source == "getName" || source.indexOf("getGoodsTypeById") != -1){
        	 	    if(index!=null){
        	 	  	 	...
        	 	  	 	if(id.id != null)
        	 	  	 		options.source="${WEB_PATH}/global/data/getGoodsTypeById.do?id=" + id.id;
        	 	  	 	else
