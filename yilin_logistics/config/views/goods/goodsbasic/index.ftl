...
<body class="gray-bg">
                                ...
                                <table id="exampleTableEvents" data-height="700" data-mobile-responsive="true" >
                                    <thead>
                                        <tr>
								            ...
								            <th data-field="specifica" data-formatter="formatSpecifica">货物规格</th>
    ...
    <script >
		...
		function formatOnLine(val){
	 	...
	 	function formatGoodsType(val){
    		return val; 
		}
		
		function formatSpecifica(val){
			return '<font class="text-danger">'+val+'</font>';
		}
		...
		function upodateGoodsBasicOnLineSuccess(result,btn) {
			...
			layer.closeAll();
			if(result.success == true){
				swal("已成功设置下线！", result.msg, "success");
				$("#exampleTableEvents").bootstrapTable('refresh');
			}else{
				swal("设置下线失败！", result.msg, "error");
				$("#exampleTableEvents").bootstrapTable('refresh');
			}
		}
