        ...
        <div class="modal-dialog">
            <div class="modal-content animated bounceInRight">
                <div class="modal-header">
                    ...
                    <!--<form class="form-horizontal validate big" id="form-card" action="${WEB_PATH }/capital/bankcard/addBandCard.do" target="_blank">--> 
                    <form class="form-horizontal validate big" id="form-card">
		                          ...
		                          <div class="form-group">
						            <label class="col-sm-3 control-label"><span style="color: red;">*&nbsp;&nbsp;</span>银行预留手机号码：</label>
						            <div class="col-md-8">
						               <input type="text" required="" aria-required="true" class="form-control" id="phone-number" name="phoneNo" maxlength="11"/>
						            </div>
						          </div>
						          <div class="form-group">
						            <label class="col-sm-3 control-label"><span style="color:red;">*&nbsp;&nbsp;</span>验证码：</label>
						             <div class="col-md-8">
							            <div class="input-group">
						                    <input type="text" id="sms-code" class="form-control" name="smsCode" maxlength="6"/>
						                    <span class="input-group-btn">
						                        <button type="button" class="btn btn-primary" onclick="sendCode(this)" id="btnCode">
...
var dt;
function onOpenCard(){
	$("#sms-code").rules("add", { required: true,number:true});
	var smsCode = $("#sms-code").val();
	var openBank = $("#openBank").val();
	if($("#form-card").valid()){
       //$("#form-card").submit();
       dt.smsCode = smsCode;
       dt.openBank = openBank;
	     $.ajax({
			type: 'POST',
			url:'${WEB_PATH }/capital/bankcard/openQueryCard.do',
...
function sendCode(obj){
	debugger;
	//$("#sms-code").rules("remove", { required: true,number:true});
	if(!$("#form-card").valid()){
		return ;
	}
	...
	$.ajax({
		  type: 'POST',
		  url:'${WEB_PATH }/capital/bankcard/getSMSCode.do',
		  data: {"cardNo":cardNo,"phoneNo":phoneNo},
		  dataType: "json",
