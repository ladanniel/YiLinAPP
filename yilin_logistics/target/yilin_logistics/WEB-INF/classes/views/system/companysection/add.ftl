                                ...
                                <div class="col-sm-8">
                                    <select id="select_id" class="form-control" name="parent_id">
                                    	<#list list as o >
                                    		...
                                    		<#if !id?? >
                                    			<option value="${o.id}" leav="${o.leav}" <#if o.parent == "0">selected</#if> >${preTemp+o.name}</option>
                                    		<#else>
                                    			<option value="${o.id}" leav="${o.leav}" <#if o.id == id>selected</#if> >${preTemp+o.name}</option>
                                    		</#if>
 ...
 $(function(){   
     //添加用户信息#commentForm
        $('#saveBut').click(function(){
        	var parent_name = $("#select_id").find("option:selected").text();
        	if(parent_name == "--- 无 ---") {
        		parent_name = "";
        	}
        	var leav = $("#select_id").find("option:selected").attr("leav");
        	if(leav == ""  || leav == null) {
        		leav = 1;
        	}else {
        		leav = parseInt(leav)+1;
        	}
        	$("input[name='parent_name']").val(parent_name);
        	$("input[name='leav']").val(leav);
			 ...
			 submitHandler: function(form) 
			   {  
			      ...
			      $.ajax({
					  type: 'POST',
					  url: '${WEB_PATH }/system/companysection/add.do',  
