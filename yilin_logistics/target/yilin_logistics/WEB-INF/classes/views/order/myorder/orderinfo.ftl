			                             ...
			                             <div class="col-sm-3"> 
			                                <div class="form-group">
			                                    ...
			                                    <select data-placeholder="选择运单状态" id="status" class="chosen-select form-control" multiple   >
				                                    ...
				                                    <option value="1" hassubinfo="true" >运输中</option>
				                                    <option value="2" hassubinfo="true" >送达</option>
				                                    <option value="3" hassubinfo="true" >回执发还中</option>
				                                    <option value="4" hassubinfo="true" >送还回执中</option>
				                                    <option value="5" hassubinfo="true">订单完结</option>
				                                </select>
			                                </div>
			                            </div>
			                        </div>
    ...
    <script >
    	...
    	$(function(){ 
	    	...
	    	$("#exampleTableEvents").bootstrapTable({
	            url: "${WEB_PATH }/order/myorder/getCompanyOrderPage.do",
	            method: 'get',
	            search:false,
	            pagination:true,
	            pageNumber:1,  
		...
		function subTableFormatter(index, row, $detail){
		 ...
		 window.operateEvents = {
	        'click .like': function (e, value, row) {
	        	var tr  = e.toElement.parentNode.parentNode.parentNode.parentNode;
	        	if($(tr).parent().find(".detail-view").length){
	        		$(tr).parent().find(".detail-view").remove();
	        	}else{
	        		var html ='<tr class="detail-view"><td colspan="9">asfasdsad</td></tr>';
	        		$(tr).after(html)
	        	}
