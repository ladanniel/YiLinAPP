<div class="modal inmodal" id="userModal" tabindex="-1" role="dialog" aria-hidden="true" style="display: block; padding-left: 6px;">
        <div class="modal-dialog">
            <div class="modal-content animated bounceInRight">
                <div class="modal-header">
                    ...
                    <h1 class="modal-title">编辑车辆轴数</h1>
                    <form class="form-horizontal m-t" id="editBearingNumInfo">
                    <input name="id" type="hidden"  class="form-control" value="${bearingNum.id}" >
                    <input name="create_time" type="hidden"  class="form-control" value="${bearingNum.create_time}" >
                    <div class="modal-body"> 
                            <div class="form-group">
                                <label class="col-sm-3 control-label"><span style="color: red;">*&nbsp;&nbsp;</span>车辆轴数：</label>
                                <div class="col-sm-8">
                                    <input id="text" name="name" minlength="2" type="text"  class="form-control" required="" aria-required="true" value="${bearingNum.name}" >
                                </div>
                            </div>
                    </div>
 ...
 $(function(){
       $('#saveBut').click(function(){
        	 $("#editBearingNumInfo").validate({     
			 submitHandler: function(form) 
			   {  
			   	  $('#saveBut').attr("disabled",true);    
				  $.yilinAjax({
				   	  	type:'POST',
				   	  	url:'${WEB_PATH }/truck/bearingNum/edit.do',
				   	  	data:$('#editBearingNumInfo').serialize(),
	            		errorcallback:null,
